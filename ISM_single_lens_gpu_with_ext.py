...
x = np.arange(v04_padded_object.field.shape[1])
y = np.arange(v04_padded_object.field.shape[0])
xx, yy = np.meshgrid(x, y)


new_dim_x = v04_padded_object.field.shape[0] * 2
new_dim_y = v04_padded_object.field.shape[1] * 2
new_x = np.linspace(0, v04_padded_object.field.shape[1]-1, new_dim_x)
new_y = np.linspace(0, v04_padded_object.field.shape[0]-1, new_dim_y)
new_xx, new_yy = np.meshgrid(new_x, new_y)

# Perform 2D interpolation
interp_func = interpolate.interp2d(x, y, v04_padded_object.field.to('cpu').numpy(), kind='cubic')
doubled_field = interp_func(new_x, new_y)


fig1, ax1 = plt.subplots()
im1 = ax1.imshow(doubled_field, extent=v04_padded_object.extent.to('cpu').numpy())
ax1.set_title('doubled_field')
plt.show(block=False)
fig1.colorbar(im1)
del fig1, ax1, im1
